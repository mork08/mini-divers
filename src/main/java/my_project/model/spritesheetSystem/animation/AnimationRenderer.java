...
public class AnimationRenderer<T extends Enum<T> & IAnimationState> {
    ...
    private final BeckerMap<T, Animation<T>> animations;

    private BeckerMap<T, Runnable> onStart;
    private BeckerMap<T, Runnable> onCycle;
    private BeckerMap<T, Runnable> onFinish;
    ...
    public AnimationRenderer(String spriteSheetPath, int rows, int maxColumns, int frameWidth, int frameHeight, int marginX, int marginY, T state) {
        BufferedImage spriteSheet = DrawTool.getNewResourceImage(spriteSheetPath);
        BeckerMap<T, Animation<T>> animations = new BeckerMap<>();
        Class<T> enumClass = state.getDeclaringClass();
        for (int i = 0; i < rows; i++) {
            BeckerList<BufferedImage> frames = new BeckerList<>();
            for (int j = 0; j < maxColumns; j++) {
                BeckerList<T> animationStates = IAnimationState.fetch(enumClass, i, j);
                if (animationStates == null || animationStates.getLength() == 0) break;
                ...
                frames.append(animationImage);

                int ref = j;
                AtomicBoolean clear = new AtomicBoolean(false);
                animationStates.forEach((animationState, index) -> {
                    if (animationState.getColumnRange().end() == ref) {
                        var f = frames.subList(0, frames.getLength());
                        if (animationState.isReverse()) f = f.reverse();
                        animations.add(animationState, new Animation<T>(animationState, f, animationState.getDuration(), animationState.isLoop(), animationState.isReverse()));
                        clear.set(true);
                    }
                });
        ...
        this.onStart = new BeckerMap<>();
        this.onCycle = new BeckerMap<>();
        this.onFinish = new BeckerMap<>();
     ...
     * @param state Start-State
     */
    public AnimationRenderer(BeckerList<Animation<T>> animations, T state) {
        BeckerMap<T, Animation<T>> map = new BeckerMap<>();
        for (int i = 0; i < animations.getLength(); i++) {
            Animation<T> animation = animations.get(i);
            map.add(animation.getState(), animation);
        }
        this.animations = map;

        if (this.hasDuplicateStates(this.animations.values())) {
            throw new InvalidParameterException("Atleast 2 framesLists have been found with the same state.");
        }
    ...
    private boolean hasDuplicateStates(BeckerList<Animation<T>> animations) {
        for (int i = 0; i < animations.getLength(); i++) {
            Animation<T> current = animations.get(i);

            for (int j = i + 1; j < animations.getLength(); j++) {
                Animation<T> other = animations.get(j);

                if (current.getState().equals(other.getState())) {
                    return true;
                }
            }
        }
    ...
    public void pauseAtEnd() {
        if (this.currentAnimation != null) {
            this.pause();
            this.currentIndex = this.currentAnimation.getFrames().getLength() - 1;
    ...
    public void switchState(T state) {
        if (this.currentAnimation == null || this.currentAnimation.getState() != state) {
            if (this.animations.get(state) != null) {
                this.currentAnimation = this.animations.get(state);
                this.currentIndex = this.currentAnimation.isReverse() ? this.currentAnimation.getFrames().getLength() - 1 : 0;
                this.elapsed = 0;
                this.paused = false;
            }
        }
    ...
    public void update(double dt) {
        if (!this.running || this.paused || this.currentAnimation == null || this.currentAnimation.getFrames().getLength() <= 1) return;
        ...
        int size = animation.getFrames().getLength();
    ...
    public void onStart(Runnable onStart) {
        this.onStart.add(this.currentAnimation.getState(), onStart);
    }
    ...
    public void onStart(T state, Runnable onStart) {
        this.onStart.add(state, onStart);
    }
    ...
    public void onCycle(Runnable onCycle) {
        this.onCycle.add(this.currentAnimation.getState(), onCycle);
    }
    ...
    public void onCycle(T state, Runnable onCycle) {
        this.onCycle.add(state, onCycle);
    }
    ...
    public void onFinish(Runnable onFinish) {
        this.onFinish.add(this.currentAnimation.getState(), onFinish);
    }
    ...
    public void onFinish(T state, Runnable onFinish) {
        this.onFinish.add(state, onFinish);
    }
     ...
     */
    public BeckerMap<T, Animation<T>> getAnimations() {
